...
{   printf("entrou");
    ...
    for( i = 0; i < tamanhostr; i++){
        ...
        if(isspace(mensagem[i]) || isalpha(mensagem[i])){
            ...
            printf("encripted %s", mensagem_encriptada[i]);
}}}
    ...
    for(k = 0; k < tamanhostr - 1; k++) {
        mensagem_encriptada[k] = power(mensagem_encriptada, e, n);//calcula x elevado a y mod p
        ...
        printf("%lld", mensagem_encriptada[k] );
